...
(function () {
  ...
  function renderDashboard() {
    ...
    const bp = [
      ...
      { v: withShips != null ? withShips.toLocaleString() : "—", k: "submitted at least once", sub: "projects with at least one ship, ever", tip: "Projects with at least one ship in their history, across everything we have ever collected — this one is about the archive, not just the counted window, so it includes projects first submitted after the cutoff." },
      { v: distinctWaiting || (b.queue_count != null ? b.queue_count : q.count), k: "projects in queue right now", sub: (s.submission_cutoff && s.submission_cutoff.projects_excluded) ? `in the counted window (+${s.submission_cutoff.projects_excluded} excluded by the cutoff)` : "waiting on a reviewer", tip: "Distinct projects with a ship currently awaiting review inside the counted window. Highlighted — this is the live line. Projects first submitted after the cutoff are excluded from it, which is why they are called out rather than silently missing.", hot: true },
    ];
